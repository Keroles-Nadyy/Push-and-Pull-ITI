...
currentuser.innerHTML = `🖥️ Current Login_User: ${username}`;
  ...
  if (message_content.logout) {
    ...
    if (message_content.private && message_content.private == username) {
      ...
      if (privateContentDiv) {
        privateContentDiv.innerHTML += `<div style="direction: rtl;"><h4 class="d-inline-block w-auto rounded" style="background: Lavender;">${message_content.content}</h4></div>`;
      }
    } else if (!message_content.private) {
      content_div.innerHTML += `<div><h4 class="d-inline-block w-auto rounded" style="background: Lavender;">${message_content.content}</h4></div>`;
    }
  }
...
function openPrivateChat(recipient) {
  ...
  privateSendButton.addEventListener("click", function () {
    ...
    privateContentDiv.innerHTML += `<div style="direction: rtl;"><h4 class="d-inline-block w-auto rounded" style="background: Lavender;">${username}: ${privateMessageInput}</h4></div>`;
  });
}
